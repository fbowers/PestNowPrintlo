...
#layout 'orderpads'
 # layout 'orders' , :except => :coversheetchecked
...
#:contractcount => route.Schedules.includes({:Orders => :Contract}).where(" Contracts.OriginatingOrderId = Orders.OrderId AND Schedules.StartTime BETWEEN ? AND ?", @start, @end).count({:Orders => :Contracts}["OriginatingOrderId"]),
 ...
 #@contracttotal = Contract.includes({:Order => :Schedule }).where("Schedules.StartTime BETWEEN ? AND ? AND Orders.Branchid = ? ", @start, @end,  @branch).count(:ContractId)
    ...
    format.pdf do
      data = DocRaptor.create(:testbulkemail => true, :name => 'orders/index.pdf', :document_content => render_to_string, :document_type => "pdf", :prince_options => {:baseurl => 'http://nofail.de'})
...
# UserMailer.welcome_email(@user).deliver
    ...
    format.pdf do
      data = DocRaptor.create(:testbulkemail => true, :name => 'orders/workorder.pdf', :document_content => render_to_string, :document_type => "pdf", :prince_options => {:baseurl => 'http://nofail.de'})
...
["Schedules.StartTime BETWEEN ? AND ? AND Orders.Branchid = ?  AND Routes.RouteId IN (?)", @start, @end,  @branch, @route]
   ...
   respond_to do |format|
    ...
    format.pdf do
      data = DocRaptor.create(:testbulkemail => true, :name => 'orders/contract.pdf', :document_content => render_to_string, :document_type => "pdf", :prince_options => {:baseurl => 'http://nofail.de'})
...
@sub = Customerledgerorder.limit(10).includes(:Order => [:Servicecode, :Schedule]).where("OrderId in (?)", @presum)
   ...
   respond_to do |format|
    ...
    format.pdf do
      data = DocRaptor.create(:testbulkemail => true, :name => 'orders/wdireport.pdf', :document_content => render_to_string, :document_type => "pdf", :prince_options => {:baseurl => 'http://nofail.de'})
...
@his = @Frank.Property.Histories.order("OrderId DESC").includes({:Schedule => :Routes}, :Servicecode).where('Schedules.StartTime is not null ')
...
@total = @rate * @count
  ...
  #  @order.update_attributes(:UpdateUserId => current_user.UserId, :UpdateTime => Time.now, :TimeIn => @tin, :TimeOut => Time.now, :CheckInResultId => @checkstatus, :TemperatureId => params[:TemperatureId], :WeatherId => params[:WeatherId],:WindId => params[:WindId])
  #  var od = {"data":{"CheckInResultId":CheckInResultId,"TemperatureId":TemperatureId,"WeatherId":WeatherId,"WindId":WindId, "WindSpeedId":WindSpeedId}};
...
def update2
  ...
  @order.update_attributes(:UpdateUserId => current_user.UserId, :UpdateTime => Time.now, :TimeIn => @tin, :TimeOut => Time.now, :CheckInResultId => @checkstatus, :TemperatureId => params[:TemperatureId], :WeatherId => params[:WeatherId],:WindId => params[:WindId],:WindSpeedId => params[:WindSpeedId], :WorkOrderNote => params[:WorkOrderNote], :CheckInNote => params[:CheckInNote],:CheckLatitude => params[:CheckLatitude], :CheckLongitude => params[:CheckLongitude])

   Customerledger.create(:EntryDate => Time.now, :InsertTime => Time.now, :Active => 1,:PropertyId => @order.PropertyId || nil, :AgentId => @order.AgentId || nil, :CustomerId => @order.CustomerId || nil,
   :SettlementCompanyId => @order.SettlementCompanyId || nil )
...
@order = Order.find(params[:id])
   @tin = params[:tin] || Time.now
  @checkstatus = params[:CheckInResultId]
...
@con2.update_attributes(:Active => 1, :Proposed => false, :StartDate => Time.now, :ServiceDate => Time.now)
             ...
             @order.Servicecode.CreatesContract == true and @recur == nil
...
@recur = @order.Contract.RecurringPrice
 #  @recur = 75
  #  @con = @order.Contract
    ...
    @soldby = @order.SoldBy || nil
    @soldbyrate = @order.Usercommissionrate.Rate || nil
    @sales = Userledger.create(:EntryDate => @now,:InsertUserId => current_user.UserId, :UserId => @soldby, :Active => 1, :Note => "Sales Recurring from Road", :OrderId => (params[:id]), :InsertTime => @now, :UpdateUserId => current_user.UserId, :UpdateTime => @now,:UpdateReason => "Reccuring Entry From Road App",
...
@sched = Railscoversheet.where(["BranchId = ? AND StartTime BETWEEN ? AND ?", @branch, @start, @end]).reorder('TimeIn DESC')
@cur = Rubyroute.where("UserId = ?", @user)
 @hope = Route.find(@cur)
@count = @sched.count()
@sum = @sched.sum("Price")
